...
_letsGetSomeWords() {
	...
	_p "hp_title : $(echo "$body" | jq '.data[0].hp_title')"
	...
	_p "hp_author : $(echo "$body" | jq '.data[0].hp_author')"
	_p "ipad_url : $(echo "$body" | jq '.data[0].ipad_url')"
	_p "hp_content : $(echo "$body" | jq '.data[0].hp_content')"
	_p "hp_makettime : $(echo "$body" | jq '.data[0].hp_makettime')"
